 ...
 */
public class ContactsRestClientIT extends AbstractContactsRestClientIT {

    @Override
    protected Contact addContact() {
        return CLIENT.target(resolveBaseUrl()).request().post(Entity.json(createContact()), Contact.class);
    }

    @Override
    protected Contact getContact(final long id) {
        return CLIENT.target(resolveBaseUrl().path(Long.toString(id))).request().get(Contact.class);
    }
