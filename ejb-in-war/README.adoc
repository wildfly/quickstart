...
The `ejb-in-war` quickstart demonstrates the deployment of an _EJB_ bean bundled in a WAR archive for deployment to {productNameFull}. The project also includes a set of Arquillian tests for the managed bean and EJB.

The example follows the common "Hello World" pattern. These are the steps that occur:

. A JSF page asks the user for their name.
. On clicking submit, the name is sent to a managed bean named `Greeter`.
