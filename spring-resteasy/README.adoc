...
:WildFlyQuickStartRepoTag: 41.0.0.Beta1
:DocInfoProductName: Red Hat JBoss Enterprise Application Platform
:DocInfoProductNameURL: red_hat_jboss_enterprise_application_platform
:DocInfoPreviousProductName: jboss-enterprise-application-platform
...
----
WARN  [org.jboss.as.dependency.private] (MSC service thread 1-6) WFLYSRV0018: Deployment "deployment.spring-resteasy.war" is using a private module ("org.jboss.vfs") which may be changed or removed in future versions without notice.
----

This warning occurs because the RESTEasy Spring integration internally uses the `org.jboss.vfs` (Virtual File System) module for classpath scanning in the {productName} environment. This is an internal implementation detail of the integration layer and does not affect application functionality.
...
These warnings occur because Spring Web includes optional async/reactive components that implement the Servlet `AsyncListener` interface. When CDI is excluded (as required for Spring applications), {productName}'s EE subsystem attempts to register these components but fails due to missing dependencies. This does not affect application functionality since these are optional features not used by this quickstart.
...
`[WARNING] * jakarta.naming.Context or InitialContext lookup.`
...
[[getting_started_with_helm_openshift]]
...
// Additional steps needed before deploying in Helm
[[deploy_helm_prerequisites_kubernetes]]
