...
1. If your server is not started, then [Start the ${product.name} server](#start-the-jboss-eap-server).
...
* Be sure to configure ${product.name} server logging as described above under [Configure the ${product.name} Server](#configure-the-jboss-eap-server). Stop the server at the end of that step.
