...
The `logging` quickstart demonstrates how to set up and log different levels of information in WildFly Application Server. An example of asynchronous logging is also included in the configuration examples.
...
1. If your server is not started, then [Start the WildFly server](#start-the-server).
...
* Be sure to configure WildFly server logging as described above under [Configure the WildFly Server](#configure-the-server). Stop the server at the end of that step.
