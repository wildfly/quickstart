...
. If your server is not started, then xref:start_the_server[Start the server].
...
[[remove_the_logging_configuration]]
...
[source,subs="+quotes,attributes+",options="nowrap"]
----
$ __{jbossHomeName}__/bin/jboss-cli.sh --connect --file=remove-logging.cli
----
NOTE: For Windows, use the `__{jbossHomeName}__\bin\jboss-cli.bat` script.
