...
In this example a service is a timer initialized by a `@Startup @Singleton` bean. 
The example is build and packaged as a single `ejb` archive.
...
To demonstrate the singleton behavior at least two application server instances need to be started.
...
The demonstration is not limited to two servers, more servers can be started by specifying a unique port offset for each one.
...
3. Use the following command to clean any possible previous build artifacts, build and deploy the `ejb` archive:

        mvn clean install wildfly:deploy

4. Ensure the `target/${project.artifactId}.jar` archive is deployed to `node1` (one without port offset) by observing the log.
...
8. Verify failover of the singleton deployment. Shutdown the server operating as the singleton master, for instance by using Ctrl + C combination in the command prompt. Observe the following messages on the node being shutdown:
   ...
   Now observe the log messages on the second server. The node will now be elected as singleton master, deployment will completely deploy and the timer will start operating:
...
Should the singleton be running on multiple nodes, the most common causes are accidentally starting the `standalone.xml` (or `stadalone-full.xml`) profile instead of `standalone-ha.xml` (or `standalone-full-ha.xml`).
Make sure to start the server with a HA profile using `-c standalone-ha.xml`.
...
Inspect content in `src/main/resources/META-INF/singleton-deployment.xml`.
Any existing deployment can be made singleton by using *deployment overlays* mechanism.
To demonstrate how to use deployment overlays, follow the steps:
...
3. Start the CLI utility and setup deployment overlay on both servers:
...
Repeat for the second server repeat with an offsetted port:

        ${jboss.home.name}-1/bin/jboss-cli.sh --connect --controller=localhost:10090
        deployment-overlay add --name=singleton-deployment --deployments=ha-singleton-deployment.jar --content=META-INF/singleton-deployment.xml=singleton-deployment.xml
        deployment-overlay redeploy-affected --name=singleton-deployment

4. Review the changes the deployment overlay in the server profile `standalone-ha.xml`:
...
5. Observe the server output, the deployments are now setup as singleton deployments.
...
For convenience the CLI scripts (`singleton-deployment-overlay-add.cli` and `singleton-deployment-overlay-remove.cli`) are located in the root directory.
