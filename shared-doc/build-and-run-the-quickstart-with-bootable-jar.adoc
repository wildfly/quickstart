...
----
ifdef::reactive-messaging[]
. Install AMQ Streams into the `bootable-jar-project` OpenShift project as outlined link:#install_amq_streams[earlier]  in this quickstart. Note that the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment variable in the pom.xml assume you have used the indicated name for your AMQ Streams cluster.
endif::[]
