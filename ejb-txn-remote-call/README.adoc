...
oc new-app --template=wildfly-s2i-chained-build-template \
  ...
  -p APPLICATION_NAME=client -p GIT_REPO=https://github.com/wildfly/quickstart.git \
  -p GIT_BRANCH=master -p GIT_CONTEXT_DIR=ejb-txn-remote-call/client
# s2i chain build for server application
oc new-app --template=wildfly-s2i-chained-build-template \
  -p IMAGE_STREAM_NAMESPACE=$(oc project -q) \
  -p APPLICATION_NAME=server -p GIT_REPO=https://github.com/wildfly/quickstart.git \
  -p GIT_BRANCH=master -p GIT_CONTEXT_DIR=ejb-txn-remote-call/server
----
...
[CAUTION]
====
Adjust the value of the `applicationImage` value in the OpenShift deployment template
`${PATH_TO_QUICKSTART_DIR}/ejb-txn-remote-call/server/server-cr.yaml` to match the project
name you are in and that application image was built into.
It is the most probably {artifactId}-project.
====
