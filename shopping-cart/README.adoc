...
. The client calls `getCartContents()` to make sure the shopping cart was removed after checkout. This results in a `jakarta.ejb.NoSuchEJBException` trace in the server, proving the cart was removed.
