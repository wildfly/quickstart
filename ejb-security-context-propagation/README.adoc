...
----
$ __{jbossHomeName}__/bin/add-user.sh -a -u 'quickstartUser' -p 'quickstartPwd1!' -g 'guest,user'
$ __{jbossHomeName}__/bin/add-user.sh -a -u 'superUser' -p 'superPwd1!' -g 'guest,user,admin'
----
...
. If you ran the script to suppress system exceptions, you should see the following configuration in the `ejb3` subsystem.
+
[source,xml,options="nowrap"]
----
...
If you did not run the script to suppress system exceptions, you should see the following exceptions in the {productName}
...
. If ran the script to suppress system exceptions, run the following command, replacing `__{jbossHomeName}__` with the path to your server.
+
[source,subs="+quotes,attributes+",options="nowrap"]
----
