...
Be sure to link:{configurePostgresCreateUserDocUrl}[Create a Database User] for the PostgeSQL database.
...
----
For Linux:  $ __{jbossHomeName}__/bin/standalone.sh -c standalone-full.xml -Djboss.tx.node.id=UNIQUE_NODE_ID_1
For Windows:  __{jbossHomeName}__\bin\standalone.bat -c standalone-full.xml  -Djboss.tx.node.id=UNIQUE_NODE_ID_1
----
...
+
----
For Linux: $ __{jbossHomeName}__/bin/jboss-cli.sh --connect --file=configure-jts-transactions.cli
For Windows: __{jbossHomeName}__\bin\jboss-cli.bat --connect --file=configure-jts-transactions.cli
----
...
. Follow the instructions to link:{configurePostgresDriverDocUrl}[Configure the PostgreSQL Driver in the {ProductName} Server] for the server 1 configuration. Be sure to pass the `-Djboss.tx.node.id=UNIQUE_NODE_ID_1` on the command line when you start the first server to configure PostgreSQL.
