...
[NOTE]
====
You may see the following warnings when the application is deployed, which are due to the database creation, and are safe to ignore:
[source]
----
WARN  [org.hibernate.engine.jdbc.spi.SqlExceptionHelper] (ServerService Thread Pool -- 84) SQL Warning Code: 0, SQLState: 00000
WARN  [org.hibernate.engine.jdbc.spi.SqlExceptionHelper] (ServerService Thread Pool -- 84) constraint "uk6m4fuhxj4ckixi1vsb8osii7r" of relation "customer" does not exist, skipping
----
====
...
[source,options="nowrap"]
----
/deployment={artifactId}-application-component-2.jar/subsystem=messaging-activemq/server=default/jms-queue=jts-quickstart:read-attribute(name=messages-added)
----
