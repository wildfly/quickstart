make sure you= helloworld-mutual-ssl: {productName} Mutual SSL(two-way) Configuration Example
...
. Open a terminal and navigate to the root of the JBoss server directory.
...
----
+
NOTE: For Windows, use the `__{jbossHomeName__\bin\standalone.bat` script.
