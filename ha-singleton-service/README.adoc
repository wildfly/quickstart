...
* Start server 2 with port offset:
+
[source,subs="+quotes,attributes+",options="nowrap"]
----
...
----
+
NOTE: For Windows, use the `__{jbossHomeName}_2__\bin\standalone.bat` script.
...
. Open a new terminal, navigate to the root directory of this quickstart, and run the following command to execute the script for Server 1. Be sure to replace {jbossHomeName}_1 with the path to the target Server 1.
+
[source,subs="+quotes,attributes+",options="nowrap"]
----
