...
==== Configuration with Bootable Jar or {productName} S2I
...
The backend is packaged wand deployed on a provisionned server:
...
[source,options="nowrap",subs="+attributes"]
----
$ helm install todo-backend --set build.ref={WildFlyQuickStartRepoTag} --set build.s2i.version={productImageVersion} todo-backend-chart
NAME: todo-backend
...
STATUS: deployed
REVISION: 1
