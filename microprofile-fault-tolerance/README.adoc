...
where `${version.server.bom}` is the version of your {productName}, in our case "{productVersion}". Now add the following dependencies:
...
Start a {productName} instance or if you have provided specific version in plugin configuration above,
use the maven plugin to provision and run on the background:
...
Add support for MicroProfile Fault Tolerance by adding the requisite extension and subsystem.
...
Since the MicroProfile Fault Tolerance subsystem modifies deployment chain, the server needs to be reloaded.
...
Why not check that our application works? Deploy the application in a running {productName} server with:
